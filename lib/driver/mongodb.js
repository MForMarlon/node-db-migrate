...
var MongodbDriver = Base.extend({
  ...
  insert: function(collectionName, toInsert, callback) {
    ...
    /**
     * Updates record(s) in a collection
     *
     * @param collectionName - the collection to update
     * @param query - the query to run
     * @param update - the update to execute
     * @param multiOption - true if the update needs to apply to all results in query; false otherwise
     * @param callback
     */
    update: function(collectionName, query, update, multiOption, callback) {
        var options = {
            query: query,
            update: update
        };
        if (!multiOption) {
            ...
            options.multi = multiOption;
        }
        return this._run('update', collectionName, options).nodeify(callback);
    },
      ...
      this.connection.connect(connectionString, function(err, db) {
        ...
        switch(command) {
          ...
          case 'insert':
            ...
            break;
          case 'update':
              db.collection(collection).update(options.query, options.update, {multi: options.multi}, callbackFunction);
              break;
          case 'remove':
            // options is the records to insert in this case
            if(util.isArray(options))
...
exports.connect = function(config, intern, callback) {
  ...
  if (config.replicaSetName !== undefined) {
    mongoString += '?replicaSet=' + config.replicaSetName;
  }
